...
import {agentState,assignDecisions,createSimulation,demoDecisions,isSheltered,snapshot,tick,BALANCE,TASK_LABELS,type Decisions,type Id,type Simulation,type Task} from '@/lib/simulation';
import './survival.css';
import {CrewTokens} from './crew-scene';
...
export default function Autonomous(){
  ...
  useEffect(()=>{ resetRun(crypto.getRandomValues(new Uint32Array(1))[0]); },[]);
  ...
  return <main className="tiny-app survival-app">
    ...
    <div className="game-grid survival-grid"><section>
      <div className={`scene survival-scene ${sim.weather==='storm'&&sim.status!=='won'?'rainy':''} ${sim.status==='paused'?'paused':''}`}>
        {sim.status!=='won'&&<div className={`forecast ${sim.weather}`}>{sim.weather==='clear'?<Sun size={18}/>:<CloudRain size={18}/>}<div><strong>{sim.weather==='clear'?'Clear skies':sim.weather==='warning'?'Storm approaching':'Storm overhead'}</strong><span>{Math.ceil(sim.weatherRemaining)}s {sim.weather==='clear'?'until warning':sim.weather==='warning'?'to reach shelter':'until clear skies'}</span></div></div>}
        <div className="scene-inner"><img className="island-art" src="/island-autonomous.png" alt="Fernwater island, with a cabin, crop garden, fishing pond, and charging dock"/><div className="night-tint"/>
          {sim.weather==='storm'&&sim.status!=='won'&&<div className="rain-overlay" aria-hidden="true">{Array.from({length:30},(_,i)=><i key={i} style={{left:`${i*37%100}%`,animationDelay:`${i%7*-.18}s`,animationDuration:`${.65+i%3*.15}s`}}/>)}</div>}
          <CrewTokens sim={sim}/>

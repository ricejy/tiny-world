...
export async function POST(request:Request){
  ...
  try{
    ...
    const questions=buildQuestions(state.robots);
    ...
    if(!parsed.success||!state.robots.every(r=>r.available_tasks.includes(parsed.data.answers[r.id].choice)&&Object.keys(parsed.data.answers[r.id].probabilities).every(task=>r.available_tasks.includes(task as typeof TASKS[number]))))return json({error:'Unexpected Jev answer. The run has been paused.'},502);
